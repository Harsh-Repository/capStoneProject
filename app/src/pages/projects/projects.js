...
import { fetchProjects } from "../../redux/wtpplSlice";

export default function Projects() {
  // const [allPojects, setAllProjects] = useState("");
  const allPojects = useSelector((state) => state);
  const [filterProjects, setFilterProjects] = useState("All");
  const dispatch = useDispatch();
  console.log(allPojects.projects.wtpplData);
      ...
      <div className="projects">
        ...
        <div className="projectCards">
          <Row xs={1} md={3} className="g-4">
            {allPojects.project.projectData &&
              allPojects.project.projectData
                .filter((project) => {
                  return (
                    filterProjects === "All" ||
